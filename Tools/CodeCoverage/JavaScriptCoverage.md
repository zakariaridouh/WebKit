...
`make-js-file-arrays.py` runs `jsmin` over it. 9,892 lines become 2,042 and there is no source map,
so a line in the report cannot be traced to `Modules/modern-media-controls/controls/slider.js`.
A coverage build passes `--no-minify` instead, from both `WebCoreMacros.cmake` and
`DerivedSources.make`, so the script keeps the concatenation's 9,892 lines; this tool does not yet
split it back into files.

It is still reported — 2,001 code lines, 727 functions, with names, under its `sourceURL` directive —
which answers "how much of the media controls ran" but not "which file". Unminified, the
concatenation offsets are recoverable exactly the way `s_JSCCombinedCode`'s are.
