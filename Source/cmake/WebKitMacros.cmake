...
macro(WEBKIT_LIBRARY _target)
    # SHARED and MODULE only. A static library is not an image, so it has no
    # __llvm_profile_filename of its own: its objects are attributed to whatever links them,
    # which is how libPAL.a's counters end up under WebCore's baked path. The five frameworks
    # go through WEBKIT_FRAMEWORK and define the symbol in a translation unit of their own, so
    # they must not get a second definition here.
    if (${_target}_LIBRARY_TYPE STREQUAL "SHARED" OR ${_target}_LIBRARY_TYPE STREQUAL "MODULE")
        WEBKIT_BAKE_COVERAGE_PROFILE_PATH(${_target})
    endif ()
...
# ENABLE_LLVM_COVERAGE bakes /private/tmp/WebKitCoverage/<Framework>_%8m%c.profraw into one
# translation unit per framework, so each dylib's counters land in a file named after it. An
# executable contains none of those translation units, so its own instrumented objects fall
# back to the profiling runtime's default.profraw in the process's working directory.
# Measured on a WebKitTestRunner layout-test run against a coverage build: 5,920 counters in
# that image, and
#
#   LLVM Profile Error: Failed to write file "default.profraw": Operation not permitted
#
# because the working directory is not the one path the sandbox allows a coverage build to
# write. Those counters were discarded on every run, silently.
#
# Generating the definition is the same mechanism the frameworks use. The alternative,
# -fprofile-instr-generate=<path>, was measured to take effect only at compile time -- naming
# it on the link line alone bakes nothing -- so it would have to go on every translation unit
# of the target, and that breaks the precompiled-header reuse in WEBKIT_REUSE_PREFIX_HEADER,
# which requires the producing and consuming targets to compile with exactly equal flags.
#
# WEBKIT_EXECUTABLE and WEBKIT_LIBRARY call this; a target declared with a bare add_library()
# has to call it itself. Set <target>_SKIP_COVERAGE_PROFILE_PATH first for a target whose
# profile nobody would collect, such as a generator that runs during the build.
#
# The path does not vary by SDK. It is correct for macOS and for every simulator, because a
# CoreSimulator runtime shares /private/tmp with the host; WebKitFeatures.cmake refuses to
# configure ENABLE_LLVM_COVERAGE for a device SDK, which is the only case where it would be
# wrong. See Tools/CodeCoverage/iOSCoverage.md for the measurements.
function(WEBKIT_BAKE_COVERAGE_PROFILE_PATH _target)
    if (NOT ENABLE_LLVM_COVERAGE OR ${_target}_SKIP_COVERAGE_PROFILE_PATH)
        return ()
    endif ()
    set(_source "${CMAKE_CURRENT_BINARY_DIR}/${_target}_CoverageProfilePath.cpp")
    file(GENERATE OUTPUT "${_source}" CONTENT
"// Generated by _WEBKIT_BAKE_COVERAGE_PROFILE_PATH in Source/cmake/WebKitMacros.cmake.
extern \"C\" char __llvm_profile_filename[] = \"/private/tmp/WebKitCoverage/${_target}_%8m%c.profraw\";
")
    # Not part of any unified bundle and not a prefix-header consumer: it is one line, and
    # keeping it out of both leaves every other target's flags and bundles untouched.
